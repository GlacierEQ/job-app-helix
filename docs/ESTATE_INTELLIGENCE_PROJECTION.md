...
`src/job_app_helix/estate_compiler.py` remains the canonical estate compiler. This layer does not replace lineage, namespace isolation, canonical-system identity, capability donors, experiment routing, receipts, or the base public-safe filter.

The projection stage consumes the canonical compiler bundle and adds only information that is contextual rather than canonical:

```text
canonical estate bundle
...
## Canonical accomplishment projection

The raw canonical graph remains preserved. The downstream accomplishment count excludes systems that are only support/reference ancestry, current experiments, unresolved lineage candidates, or archived roots. This is a projection over the canonical graph, not a rewrite of canonical identity.

## Role relevance

The fifth promotion-score dimension is recalculated per company from capability overlap with declared target roles. Role profiles are policy-defined and use capability IDs already emitted by the canonical Capability Donor Registry.
...
The canonical `public_safe_projection()` remains the first public filter. The intelligence projection then tightens it further by omitting support-only systems, experiments, unresolved-lineage systems, private identities, legal-private records, and raw estate cardinalities.
