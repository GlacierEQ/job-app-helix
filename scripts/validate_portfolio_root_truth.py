...
def validate() -> dict[str, Any]:
    ...
    require(root.get("truth_model", {}).get("fail_closed_on_missing_or_unsupported_evidence") is True, "root must fail closed")
    ...
    require(all(isinstance(name, str) and name for name in workspace), "inventory repository names must be nonempty strings")
    require(len(workspace) == len(set(workspace)), "inventory contains duplicate repository identities")
    total = inventory.get("total_repositories")
    require(total == len(workspace) + 1, f"inventory total {total!r} does not equal root plus {len(workspace)} children")
    ...
    for flagship in flagship_rows:
        ...
        if repository is not None:
            require(isinstance(repository, str) and repository.startswith("GlacierEQ/"), f"invalid flagship repository: {repository!r}")
    ...
    require(isinstance(required_tracks, list) and required_tracks, "required_company_tracks must be nonempty")
    require(len(required_tracks) == len(set(required_tracks)), "required_company_tracks contains duplicates")
    ...
    for relative in dossier_files:
        ...
        for raw_company in companies:
            ...
            require(isinstance(company_id, str) and company_id, f"{relative}: company_id is required")
            ...
            for row in repositories:
                require(isinstance(row, list) and len(row) == 6, f"{company_id}: repository rows must contain six columns")
                repository, level, promotion_state, visibility, inventory_scope, provenance = row
                require(isinstance(repository, str) and repository.startswith("GlacierEQ/"), f"{company_id}: invalid repository {repository!r}")
                require(level in {"L0", "L1", "L2", "L3", "L4", "L5"}, f"{repository}: invalid level {level!r}")
                require(visibility in {"public", "private"}, f"{repository}: invalid visibility {visibility!r}")
                require(inventory_scope in {"HELIX_ADMITTED", "ESTATE_DISCOVERED_NOT_HELIX_ADMITTED"}, f"{repository}: invalid inventory scope")
                require(isinstance(promotion_state, str) and promotion_state, f"{repository}: missing promotion state")
                require(isinstance(provenance, str) and provenance, f"{repository}: missing provenance")
                ...
                if inventory_scope == "HELIX_ADMITTED":
                    ...
                    require(name in workspace_set, f"HELIX_ADMITTED repository is absent from canonical inventory: {repository}")
    ...
    require(not unmapped_inventory, f"canonical inventory children missing governed company/core mapping: {unmapped_inventory}")
