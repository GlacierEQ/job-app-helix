...
def load_company_catalog(
    ...
    if not isinstance(shard_paths, list) or not all(
        ...
        for raw_company in records:
            ...
            for raw_repository in repositories:
                if isinstance(raw_repository, list):
                    if len(raw_repository) != len(columns):
                        raise ExperienceGraphError(
                            f"Repository column mismatch for {company_id}"
                        )
                    ...
                    raise ExperienceGraphError(
                        f"Invalid repository record for {company_id}"
                    )
                full_name = repository.get("repository")
                if not isinstance(full_name, str):
                    raise ExperienceGraphError(
                        f"Repository identity missing for {company_id}"
                    )
                ...
                if existing is not None and existing["company_id"] != company_id:
                    raise ExperienceGraphError(
                        f"Repository mapped to multiple companies: {full_name}"
                    )
...
def _company_aliases(
    ...
    for company_id, company in companies.items():
        values = {
            ...
            normalize(str(company["display_name"])),
        }
        repository_prefixes = Counter(
            normalize(repository_name(item["repository"])).split("-", maxsplit=1)[0]
            for item in company["repositories"]
        )
        values.update(
            prefix for prefix, count in repository_prefixes.items() if count >= 2
        )
...
def _paradigm_catalog(policy: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for paradigm_id, value in raw.items():
        ...
        if not all(isinstance(item, str) for item in aliases):
            raise ExperienceGraphError(f"Invalid paradigm aliases: {paradigm_id}")
        paradigms[paradigm_id] = {
            "paradigm_id": paradigm_id,
            "display_name": display_name,
            "aliases": sorted({normalize(item) for item in aliases}),
            "repositories": [],
        }
    return paradigms
...
def _resolve_company(
    ...
    matches = [
        (len(alias), company_id)
        for company_id, company_aliases in aliases.items()
        for alias in company_aliases
        if _matches(name, alias)
    ]
    if not matches:
        return None
...
def _resolve_paradigms(
    ...
    return sorted(
        paradigm_id
        for paradigm_id, paradigm in paradigms.items()
        if any(_matches(name, alias) for alias in paradigm["aliases"])
    )
...
def build_experience_graph(
    *,
    ...
    generic_prefixes = {
        normalize(value) for value in policy.get("generic_prefixes", [])
    }
    ...
    for paradigm_id, paradigm in sorted(paradigms.items()):
        nodes.append(
            {
                "id": f"paradigm:{paradigm_id}",
                "kind": "paradigm",
                "paradigm_id": paradigm_id,
                "label": paradigm["display_name"],
                "state": "POLICY_CONFIRMED",
            }
        )
        ...
        nodes.append(
            {
                ...
                "promotion_state": evidence.get("promotion_state", "UNCLASSIFIED"),
                "evidence_level": evidence.get("skill_innovation_level"),
                "provenance_state": evidence.get("provenance_state", "UNCLASSIFIED"),
            }
        )
    ...
    for flagship in flagship_records:
        ...
        if not isinstance(system_id, str):
            raise ExperienceGraphError("Flagship has no system_id")
        compiled = {
            "system_id": system_id,
            "repository": flagship.get("repository"),
            "level": flagship.get("level"),
        ...
        node = {"id": f"flagship:{system_id}", "kind": "flagship", **compiled}
        nodes.append(node)
        repository = flagship.get("repository")
        if isinstance(repository, str):
            edges.append(
                {
                    "source": f"flagship:{system_id}",
                    "target": f"repo:{repository}",
        ...
        company_output.append(
            {
                ...
                "track_state": company.get("track_state"),
                "catalog_repository_count": len(company["repositories"]),
    ...
    claimed_aliases = {
        alias for values in company_aliases.values() for alias in values
    } | {
        alias
    ...
    family_candidates = [
        ...
        for prefix, members in sorted(family_members.items())
        if len(members) >= minimum_family_size and prefix not in claimed_aliases
    ]
    ...
    payload: dict[str, Any] = {
        ...
        "truth_boundary": {
            ...
            "unknown_families_require_review": True,
        },
        "counts": {
            "repositories": len(repositories),
            "companies": len(company_output),
            "flagships": len(compiled_flagships),
            "confirmed_paradigms": len(paradigms),
            "unclassified_family_candidates": len(family_candidates),
        },
        "companies": company_output,
        "flagships": compiled_flagships,
        "paradigms": [
            {
                "paradigm_id": paradigm_id,
                "display_name": paradigm["display_name"],
                "repository_count": len(set(paradigm["repositories"])),
        ...
        "graph": {
            "internal": {"nodes": nodes, "edges": edges},
            "public": {"nodes": public_nodes, "edges": public_edges},
        },
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compile company, flagship, and paradigm projections from live inventory"
    )
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(
        "Experience graph compiled: "
        f"snapshot={graph['snapshot_id']} repositories={graph['counts']['repositories']}"
    )
    return 0
