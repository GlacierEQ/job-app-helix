...
DEFAULT_APPLICATION_DB = Path("artifacts/application-operations.sqlite3")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="job-app-helix",
        description="Run evidence-bound job targeting and application operations.",
    )
    ...
    ingest = sub.add_parser("ingest", help="ingest a job opening from JSON or a live URL")
    source = ingest.add_mutually_exclusive_group(required=True)
    source.add_argument("--file", type=Path, help="JSON job-opening record")
    source.add_argument("--url", help="URL containing JSON or JSON-LD JobPosting data")
    ingest.add_argument("--output", type=Path, help="write normalized opening JSON")
    ingest.add_argument("--db", type=Path, default=DEFAULT_APPLICATION_DB)
    ingest.add_argument("--json", action="store_true")

    project = sub.add_parser(
        "project",
        help="match a real opening and compile resume, cover letter, outreach, and packet",
    )
    project.add_argument("company", help="mapped company id or display name")
    project.add_argument("--opening", type=Path, required=True, help="normalized job opening JSON")
    project.add_argument("--profile", type=Path, required=True, help="evidence-only candidate profile")
    project.add_argument("--role", help="mapped target role")
    project.add_argument("--output-dir", type=Path, default=Path("artifacts/applications"))
    project.add_argument("--db", type=Path, default=DEFAULT_APPLICATION_DB)
    project.add_argument("--json", action="store_true")

    applications = sub.add_parser("applications", help="list persisted application state")
    applications.add_argument("--db", type=Path, default=DEFAULT_APPLICATION_DB)
    applications.add_argument("--json", action="store_true")

    transition = sub.add_parser("transition", help="move a persisted application state")
    transition.add_argument("application_id")
    transition.add_argument("status")
    transition.add_argument("--external-reference")
    transition.add_argument("--note", default="")
    transition.add_argument("--db", type=Path, default=DEFAULT_APPLICATION_DB)

    response = sub.add_parser("response", help="record a response and update lifecycle state")
    response.add_argument("application_id")
    response.add_argument("kind", choices=("interview", "offer", "rejection", "other"))
    response.add_argument("--note", required=True)
    response.add_argument("--source-reference")
    response.add_argument("--db", type=Path, default=DEFAULT_APPLICATION_DB)

    feedback = sub.add_parser("feedback", help="record outcome feedback for future iteration")
    feedback.add_argument("application_id")
    feedback.add_argument("outcome")
    feedback.add_argument("--note", required=True)
    feedback.add_argument("--db", type=Path, default=DEFAULT_APPLICATION_DB)

    summary = sub.add_parser("feedback-summary", help="show lifecycle outcome counts")
    summary.add_argument("--db", type=Path, default=DEFAULT_APPLICATION_DB)

    demo = sub.add_parser("demo", help="run the legacy deterministic campaign fixture")
...
def _run_ingest(args: argparse.Namespace) -> int:
    opening = load_job_opening(args.file) if args.file else ingest_job_opening_url(args.url)
    with ApplicationStore(args.db) as store:
        store.save_opening(opening)
    if args.output:
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(
            json.dumps(opening.as_dict(), indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
    if args.json:
        print(json.dumps(opening.as_dict(), indent=2, sort_keys=True))
    else:
        print(
            f"INGESTED {opening.opening_id} | {opening.company} | "
            f"{opening.title} | digest={opening.digest}"
        )
    return 0
...
def _run_project(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        print(
            ...
            f"match={packet['match']['overall_score']:.3f} | "
            f"adapter={packet['adapter_receipt']['adapter']}"
        )
...
def _run_applications(args: argparse.Namespace) -> int:
    with ApplicationStore(args.db) as store:
        rows = store.list_applications()
    if args.json:
        print(json.dumps({"applications": rows}, indent=2, sort_keys=True))
    else:
        for row in rows:
            print(
                f"{row['application_id']} {row['status']:10} "
                f"{row['company_id']} | {row['role']}"
            )
    return 0
...
def _run_transition(args: argparse.Namespace) -> int:
    with ApplicationStore(args.db) as store:
        ...
        row = store.get_application(args.application_id)
...
def _run_feedback(args: argparse.Namespace) -> int:
    with ApplicationStore(args.db) as store:
        store.record_feedback(args.application_id, args.outcome, args.note)
        events = store.events(args.application_id)
