...
def parse_company_intelligence(
    ...
    if manifest.get("schema") != EXPECTED_SCHEMA:
        raise ValueError("external company-intelligence manifest schema is invalid")
    refs = manifest.get("shards")
    count = manifest.get("record_count")
    if not isinstance(refs, list) or not isinstance(count, int):
        raise ValueError("external company-intelligence manifest is incomplete")
    boundary = manifest.get("inference_boundary")
    if not isinstance(boundary, str) or not boundary:
        raise ValueError("external company intelligence requires an inference boundary")

    output: dict[str, dict[str, Any]] = {}
    for index, ref in enumerate(refs):
        if not isinstance(ref, dict) or not isinstance(ref.get("path"), str):
            raise ValueError(f"invalid intelligence shard reference at index {index}")
        path = ref["path"]
        ...
        if isinstance(declared_count, int) and declared_count != len(records):
            raise ValueError(f"intelligence shard count mismatch: {path}")
        ...
        if (
            ...
            raise ValueError(f"intelligence shard digest mismatch: {path}")

        for raw in records:
            if not isinstance(raw, dict):
                raise ValueError(f"invalid company-intelligence row in {path}")
            company_id = raw.get("company_id")
            if not isinstance(company_id, str) or not company_id:
                raise ValueError(f"company-intelligence row has no company_id in {path}")
            if company_id in output:
                raise ValueError(f"duplicate company-intelligence row: {company_id}")
            ...
            for field in ("mechanism", "expected_impact"):
                if not isinstance(leverage.get(field), str) or not leverage[field].strip():
                    raise ValueError(f"{company_id} lacks leverage.{field}")

            sources = raw.get("official_sources")
            if not isinstance(sources, list) or not sources:
                raise ValueError(f"{company_id} lacks official sources")
            normalized_sources: list[dict[str, Any]] = []
            for source in sources:
                if not isinstance(source, dict):
                    raise ValueError(f"{company_id} has invalid official source")
                url = source.get("url")
                source_sha = source.get("source_sha256")
                title = source.get("title")
                signal = source.get("observed_signal")
                if not isinstance(url, str) or not url.startswith("https://"):
                    raise ValueError(f"{company_id} has invalid official source URL")
                if not isinstance(source_sha, str) or SOURCE_SHA_RE.fullmatch(source_sha) is None:
                    raise ValueError(f"{company_id} has invalid official source hash")
                if not isinstance(title, str) or not title:
                    raise ValueError(f"{company_id} has invalid official source title")
                if not isinstance(signal, str) or not signal:
                    raise ValueError(f"{company_id} has invalid observed signal")
    ...
    if len(output) != count:
        raise ValueError(f"company-intelligence count mismatch: {len(output)} != {count}")
    ...
    if overlap:
        raise ValueError(f"excluded company intelligence leaked into snapshot: {overlap}")
    return output
