...
class ManualApplicationAdapter:
    """Create a human-submittable packet without claiming submission."""
...
def ingest_job_opening(
    ...
    *,
    ...
    company = str(
        value.get("company")
        or value.get("hiringOrganization")
        or ""
    ).strip()
    ...
    if not company or not title or not description:
        raise ValueError(
            "job opening requires company, title, and description"
        )

    location = _location_text(
        value.get("location") or value.get("jobLocation")
    )
    requirements = _strings(
        value.get("requirements") or value.get("qualifications")
    )
    preferred = _strings(
        value.get("preferred")
        or value.get("preferredQualifications")
    )
    canonical = {
        "company": company,
        "title": title,
...
def match_opening(
    ...
    *,
    ...
    role_score = len(role_tokens & title_tokens) / max(
        1,
...
def _resume_markdown(
    ...
    opening_text = (
        opening.description
        + " "
        + " ".join(opening.requirements)
    )
    relevant = [
        skill
        for skill in profile.skills
        if _tokens(skill) & _tokens(opening_text)
    ]
    if not relevant:
        relevant = list(profile.skills[:8])

    experience = "\n".join(
        f"- {item}" for item in profile.experience
    )
    if not experience:
        experience = (
            "- No experience bullets supplied in candidate profile."
        )
    achievements = "\n".join(
        f"- {item}" for item in profile.achievements
    )
    if not achievements:
        achievements = (
            "- No achievement bullets supplied in candidate profile."
        )
    proof = "\n".join(
        f"- {row['repository']}"
    ...
    if not skills:
        skills = "No skills supplied in candidate profile."
...
def _cover_letter_markdown(
    ...
    paragraphs = [
        (
            f"# Cover Letter — {target.display_name} / "
            f"{opening.title}"
        ),
        "Dear Hiring Team,",
        (
            f"I am applying for the {opening.title} role. "
            f"{profile.summary}"
        ),
        (
            "My strongest evidence for this application is "
            "concrete work rather than unsupported claims. "
            f"{kit.recruiter_thesis}"
        ),
        "Public proof relevant to this application:\n" + proof,
        (
            f"The role is a {recommendation} against the supplied "
            "profile and job description "
            f"(match score {match.overall_score:.2f}). "
            "I would welcome the opportunity to discuss the systems "
            "and implementation decisions behind this work."
        ),
        f"Sincerely,\n{profile.name}",
...
def _outreach_markdown(
    ...
    paragraphs = [
        ...
        (
            f"I'm applying for {opening.title} at "
            f"{target.display_name}. My work centers on {summary}. "
            f"The most relevant public proof for this role is {proof}. "
            "I'd value a conversation about where that evidence maps "
            "to the team's current bottlenecks."
        ),
        kit.non_affiliation,
    ]
    return "\n\n".join(paragraphs).rstrip() + "\n"
...
def project_application(
    ...
    *,
    ...
    if not kit.proof_repositories:
        raise ValueError(
            "cannot project application without admitted public proof"
        )
    ...
    resume = _resume_markdown(
        opening,
        target,
        kit,
        profile,
    )
    cover = _cover_letter_markdown(
        opening,
        target,
        kit,
        profile,
    ...
    projection = Projection(
        ...
        claim_sources=tuple(
            [
                f"candidate-profile:{profile.source_digest}",
                *proof_repos,
            ]
        ),
        digest=digest,
    ...
    def transition(
        ...
        *,
        ...
        if (
            ...
            raise ValueError(
                "SUBMITTED requires external_reference"
            )
