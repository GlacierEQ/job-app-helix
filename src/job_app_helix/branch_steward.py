...
def _run(repo: Path, *args: str, check: bool = True) -> subprocess.CompletedProcess[str]:
    ...
    if check and completed.returncode != 0:
        raise BranchStewardError(f"git {' '.join(args)} failed ({completed.returncode}): {completed.stderr.strip()}")
    return completed
...
def assess_branch(repo: Path, reference: str, branch: str) -> BranchAssessment:
    """Assess branch ancestry without converting ancestry into retirement authority.

    This function cannot declare a branch retirement-ready. Git ancestry and patch
    equivalence are overlap evidence only. Retirement requires provider-read-back
    UNIQUE_CONTRIBUTION=0 across the donor's source-bearing value and durable lineage
    preservation. That evidentiary boundary does not manufacture an additional
    per-retirement approval requirement where controlling Operator instructions govern.
    """
    repo = repo.resolve()
    ...
    if ahead == 0:
        ...
        reason = "branch has no commits absent from reference ancestry, but ancestry alone cannot establish capability exhaustion; inspect purpose, lineage, artifacts, consumers, and historical donor value before any lifecycle decision"
    elif not unique:
        classification = "PATCH_EQUIVALENT_CAPABILITY_REVIEW_REQUIRED"
        safe_direct_merge = False
        reason = "git-cherry found no unique patch commits, but patch equivalence is not proof of functional or historical redundancy; perform capability review"
    elif behind == 0:
        classification = "CURRENT_UNIQUE_VALUE"
        safe_direct_merge = True
        reason = "branch is based on current ancestry and contains unique patches; verify and compose its useful capability into the strongest current system"
    else:
        classification = "DIVERGED_UNIQUE_VALUE"
        safe_direct_merge = False
        reason = "branch is behind current ancestry and contains unique patches; synthesize its useful delta with later gains on fresh ancestry rather than discarding either side"
    return BranchAssessment(repository=repo.name, reference_branch=reference, branch=branch, merge_base=merge_base, ahead=ahead, behind=behind, unique_patch_commits=unique, changed_files=files, classification=classification, safe_direct_merge=safe_direct_merge, retirement_ready=False, operator_review_required=False, capability_review_required=True, reason=reason)


def list_remote_branches(repo: Path, reference: str = "main", remote: str = "origin", protected_prefixes: Sequence[str] = ("upstream", "vendor", "release/")) -> tuple[str, ...]:
    rows = _line_output(repo, "for-each-ref", "--format=%(refname:short)", f"refs/remotes/{remote}/")
    ...
    for row in rows:
        ...
        if branch in {"HEAD", reference} or branch.casefold().startswith(protected):
            continue
        out.append(branch)
    return tuple(sorted(set(out), key=str.casefold))


def assess_repository(repo: Path, reference: str = "main", remote: str = "origin") -> dict[str, object]:
    ...
    priority_order = {"DIVERGED_UNIQUE_VALUE": 0, "CURRENT_UNIQUE_VALUE": 1, "PATCH_EQUIVALENT_CAPABILITY_REVIEW_REQUIRED": 2, "ANCESTRY_EQUIVALENT_CAPABILITY_REVIEW_REQUIRED": 3}
    assessments.sort(key=lambda item: (priority_order.get(item.classification, 99), -len(item.unique_patch_commits), item.branch.casefold()))
    return {
        ...
        "remote_ref_deletion_allowed": False,
