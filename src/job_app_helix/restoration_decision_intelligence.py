...
class IntelligentRestorationCandidate:
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> IntelligentRestorationCandidate:
        ...
        fields = (
            "market_demand", "differentiation", "evidence_strength",
            "strategic_fit", "time_to_value", "reversibility",
        )
        for field in fields:
        ...
        return cls(
            base=base, estimated_effort=effort, dependencies=dependencies,
            conflicts=conflicts, **extras,
        )

    def metric(self, name: str) -> float:
        source = self.base if hasattr(self.base, name) else self
        return float(getattr(source, name))


def _scenario_score(
    candidate: IntelligentRestorationCandidate, weights: Mapping[str, float]
) -> float:
    return sum(candidate.metric(field) * weight for field, weight in weights.items())


def rank_intelligent_restoration_queue(
    raw_candidates: list[Mapping[str, Any]], *, completed: tuple[str, ...] = ()
) -> list[dict[str, Any]]:
    ...
    for row in candidates:
        ...
        if unknown:
            raise RestorationRoutingError(
                f"{row.base.candidate_id} has unknown dependencies: {sorted(unknown)}"
            )
    completed_set = set(completed)
    feasible = [
        row for row in candidates
        if not row.base.blocked and set(row.dependencies).issubset(completed_set)
    ]
    if not feasible:
    ...
    scores = {
        row.base.candidate_id: {
            name: _scenario_score(row, weights) for name, weights in SCENARIOS.items()
        }
        for row in active
    }
    ...
    for row in candidates:
        ...
        if row in active:
            ...
            wins = sum(
                sum(
                    scenario[name] > scores[other.base.candidate_id][name]
                    for name in SCENARIOS
                ) >= 2
                for other in active
                if other.base.candidate_id != cid
            )
            avg = mean(scenario.values())
            robust = avg - 0.35 * regret + 0.20 * wins
            reasons = [
                "active highest-priority executable tier",
                f"worst-case regret={regret:.3f}",
            ...
            if row.base.blocked:
                ...
                reason = f"deferred behind P{best_tier}"
            reasons = [reason]
        ranked.append({
            "candidate_id": cid, "title": row.base.title,
            "repository": row.base.repository, "tier": row.base.tier,
            "robust_score": round(robust, 4), "mean_utility": round(avg, 4),
            "worst_case_regret": round(regret, 4), "outranking_wins": wins,
            "scenario_scores": {k: round(v, 4) for k, v in scenario.items()},
            "dependencies_satisfied": deps_ok, "blocked": row.base.blocked,
            "reasons": reasons,
        })
...
def compile_intelligent_restoration_decision(
    ...
    result: dict[str, Any] = {
        ...
        "mechanisms": [
            "priority_precedence", "scenario_analysis", "minimax_regret",
            "pairwise_outranking", "dependency_feasibility",
        ],
        "donor": dict(MEGAMIND_V2_DONOR),
        "fallback": {
            "schema": "glaciereq.job-restore.megamind-routing.v1",
            "winner": rank_restoration_queue(raw)[0].candidate_id,
        },
    }
    if "effort_budget" not in payload:
        return result
    budget = float(payload["effort_budget"])
    if budget <= 0:
        raise RestorationRoutingError("effort_budget must be positive")
    pool = [IntelligentRestorationCandidate.from_mapping(row) for row in raw]
    selected: list[str] = []
    spent = 0.0
    while True:
        completed_now = set(completed) | set(selected)
        eligible = [
            row for row in pool
            if row.base.candidate_id not in selected
            and not row.base.blocked
            and set(row.dependencies).issubset(completed_now)
            and row.estimated_effort + spent <= budget
            and not (set(row.conflicts) & set(selected))
        ]
        if not eligible:
            break
        eligible_ids = {row.base.candidate_id for row in eligible}
        eligible_raw = [row for row in raw if row["candidate_id"] in eligible_ids]
        winner = rank_intelligent_restoration_queue(
            eligible_raw, completed=tuple(completed_now)
        )[0]["candidate_id"]
        chosen = next(row for row in eligible if row.base.candidate_id == winner)
        selected.append(winner)
        spent += chosen.estimated_effort
    result["plan"] = {
        "selected_ids": selected, "effort_budget": budget,
        "effort_spent": round(spent, 4), "effort_remaining": round(budget - spent, 4),
    }
    return result
