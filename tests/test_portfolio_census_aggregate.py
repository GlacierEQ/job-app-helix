...
def test_aggregate_reconciles_exact_inventory_and_action_queues(
    ...
    assert summary["workspace"]["admission_classes"] == {
        "candidate_attributed_downstream": 1,
        "candidate_public_unresolved_provenance": 1,
    }
    assert summary["workspace"]["provenance_states"] == {
...
def test_aggregate_fails_on_missing_expected_receipt(tmp_path: Path) -> None:
    ...
    try:
        module.build_summary(tmp_path, inventory())
    except ValueError as exc:
        assert "GlacierEQ/downstream" in str(exc)
    else:
        raise AssertionError("Missing census receipt did not fail closed")
...
def test_aggregate_fails_on_duplicate_repository_receipt(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert "Duplicate census receipt" in str(exc)
    else:
        raise AssertionError("Duplicate census receipt did not fail closed")
...
def test_aggregate_rejects_legacy_audit_schema(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert "Unexpected census schema" in str(exc)
    else:
        raise AssertionError("Legacy census schema did not fail closed")
