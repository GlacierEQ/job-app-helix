...
from job_app_helix.opening_evidence_freshness import (
    census_opening_evidence_freshness,
...
def _receipt(packet_dir: Path, observed_at: str | None) -> None:
    ...
    (packet_dir / "OPENING_INPUT_RECEIPT.json").write_text(
        json.dumps(payload),
        encoding="utf-8",
    )


def test_reverification_plan_is_actionable_without_mutating_packet_evidence(
    tmp_path: Path,
) -> None:
...
def test_census_marks_unobserved_active_packet_for_source_observation(
    tmp_path: Path,
) -> None:
    _receipt(tmp_path / "packet-a", None)
    _receipt(tmp_path / ".stale" / "prior-packet", "2026-08-20T08:00:00Z")

    census = census_opening_evidence_freshness(
        tmp_path,
