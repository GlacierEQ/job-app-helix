...
WAVE13 = ROOT / "manifests/public_repository_surface_reconciliation_wave13_2026-08-11.json"
...
def report_through_wave12() -> dict:
...
def test_weather_admission_is_scope_and_receipt_bound() -> None:
    ...
    assert record["decision_excellence_state"] == "LOCAL_ENVIRONMENTAL_RULE_GATE_VERIFIED"
    ...
    assert evidence["evidence_token"] == (
        "LOCAL_ENVIRONMENTAL_RULE_GATE_NOT_LAUNCH_SAFETY_AUTHORITY"
    )
...
def test_wave13_receipt_matches_reconciliation_authority() -> None:
    wave = load(WAVE13)["items"][0]
...
def test_wave13_admission_fails_closed_on_receipt_drift() -> None:
    predecessor = report_through_wave12()
    mismatch = deepcopy(load(WAVE13))
