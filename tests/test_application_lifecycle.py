    ...
    return ingest_job_opening(
        {
            ...
            "description": (
                "Build reliable AI safety systems, Python services, evaluation pipelines, "
                "distributed infrastructure, observability, and failure recovery."
            ),
        ...
        json.dumps(
            {
                ...
                "summary": (
                    "I build evidence-grounded automation, AI evaluation, and reliable "
                ...
                "experience": [
                    "Built execution systems with deterministic receipts and failure recovery.",
                    "Designed full-stack automation and evidence-grounded data workflows.",
                ],
                "achievements": [
                    "Developed public technical repositories demonstrating agent coordination."
                ],
...
def test_matching_uses_candidate_evidence_and_public_proof(tmp_path: Path) -> None:
    profile = load_candidate_profile(_profile_file(tmp_path))
    target = find_target("anthropic", _targets())
    match = match_opening(_opening(), target, profile, mapped_role="Safety Systems Engineer")
...
def test_projection_uses_only_profile_claims_and_admitted_repositories(tmp_path: Path) -> None:
    profile = load_candidate_profile(_profile_file(tmp_path))
    target = find_target("anthropic", _targets())
    kit, _, projection = project_application(
        _opening(), target, profile, role="Safety Systems Engineer"
    )
    assert "Built execution systems with deterministic receipts" in projection.resume_markdown
    ...
    assert any(source.startswith("candidate-profile:") for source in projection.claim_sources)
    assert all(
...
def test_full_lifecycle_writes_packet_persists_state_and_tracks_response(tmp_path: Path) -> None:
    profile = load_candidate_profile(_profile_file(tmp_path))
    target = find_target("anthropic", _targets())
    ...
    with ApplicationStore(db) as store:
        ...
        assert packet["adapter_receipt"]["status"] == "READY_FOR_MANUAL_SUBMISSION"
        assert packet["adapter_receipt"]["submission_performed"] is False
        for path in packet["artifacts"].values():
            assert Path(path).is_file()
        store.transition(application_id, "SUBMITTED", external_reference="ats-123")
        ...
        store.record_feedback(application_id, "positive", "Interview requested.")
        assert store.get_application(application_id)["status"] == "INTERVIEW"
        event_types = [event["event_type"] for event in store.events(application_id)]
        assert event_types == [
            "CREATED",
            "STATUS_CHANGED",
            "RESPONSE",
...
def test_submitted_state_requires_external_receipt(tmp_path: Path) -> None:
    ...
    with ApplicationStore(tmp_path / "operations.sqlite3") as store:
        packet = compile_application_lifecycle(
            _opening(), target, profile, output_dir=tmp_path / "out", store=store
        )
        with pytest.raises(ValueError, match="external_reference"):
            store.transition(packet["application_id"], "SUBMITTED")


def test_json_api_adapter_is_dry_run_until_explicit_submission(tmp_path: Path) -> None:
    ...
    try:
        ...
        packet = {"application_id": "app-1", "role": "Safety Systems Engineer"}
    ...
    assert received == [{"application_id": "app-1", "role": "Safety Systems Engineer"}]
