from job_app_helix.restoration_decision_intelligence import (
    compile_intelligent_restoration_decision,
)
...
def test_recruiter_demand_can_break_old_utility_tie():
    wanted = candidate(
        "wanted", market_demand=10, differentiation=10, recruiter_leverage=10
    )
    ordinary = candidate("ordinary")
    result = compile_intelligent_restoration_decision(
        {"candidates": [ordinary, wanted]}
    )
...
def test_priority_precedence_survives_scenario_intelligence():
    ...
    p1 = candidate(
        "p1", market_demand=10, differentiation=10, recruiter_leverage=10
    )
    result = compile_intelligent_restoration_decision({"candidates": [p1, p0]})
    assert result["winner"]["candidate_id"] == "p0"
...
def test_dependency_and_budget_plan_compose_work():
    ...
    result = compile_intelligent_restoration_decision(
        {"candidates": [foundation, multiplier], "effort_budget": 2}
    )
